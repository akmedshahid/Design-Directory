...
import { Home, Globe, Search, Bookmark, Library, PlusCircle, Settings, ShieldAlert, ChevronRight, FolderClosed, Compass, ShoppingBag } from 'lucide-react';
...
const Sidebar = () => {
  ...
  const isSitesActive = location.pathname.includes('/app/sites');
    ...
    <aside className="sidebar">
      ...
      <div className="sidebar-content">
        <div className="sidebar-nav-section">
          ...
          <NavLink to="/app/sites" end={false} className={({isActive}) => `nav-item ${isSitesActive ? 'active' : ''}`}>
            <Globe size={18} />
            <span className="nav-label">Sites</span>
          </NavLink>
          <NavLink to="/app/great-hall" className={({isActive}) => `nav-item ${isActive ? 'active' : ''}`}>
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>
            <span className="nav-label">Great Hall</span>
          </NavLink>
          <NavLink to="/app/group-buys" className={({isActive}) => `nav-item ${isActive ? 'active' : ''}`}>
            <ShoppingBag size={18} />
